...
feature -- Test routines
	...
	test_multiple_items
		...
		do
			create seq.make
			create s1.make ("first")
			create s2.make ("second")
			create s3.make ("third")
			seq.extend (s1)
			seq.extend (s2)
			seq.extend (s3)
			assert ("count_three", seq.count = 3)
			assert ("item_1", seq.item (1) = s1)
			assert ("item_2", seq.item (2) = s2)
			assert ("item_3", seq.item (3) = s3)
	...
	test_put
		...
		do
			...
			create s1.make ("first")
			create s2.make ("second")
			create replacement.make ("replaced")
			seq.extend (s1)
			seq.extend (s2)
			seq.put (replacement, 1)
			assert ("replaced", seq.item (1) = replacement)
		end
	...
	test_remove
		...
		do
			...
			seq.remove (1)
			assert ("count_one", seq.count = 1)
			assert ("remaining_is_s2", seq.item (1) = s2)
		end
	...
	test_wipe_out
		...
		do
			create seq.make
			seq.extend (create {YAML_STRING}.make ("item"))
	...
	test_iteration
		...
		do
			...
			seq.extend (create {YAML_STRING}.make ("c"))
	...
	test_nested_sequence
		...
		do
			create outer.make
			create inner.make
			inner.extend (create {YAML_STRING}.make ("nested"))
			outer.extend (inner)
