note
	description: "Summary description for {FILE_NATURAL_8_INPUT_STREAM}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature {NONE} -- Initialization
	...
	make (a_file: FILE)
		...
		do
			...
			create current_chunk.make_empty (0)
			count := a_file.count
			chunk_size := default_chunk_size.min (count)
			source := a_file
			start
		end
...
feature -- Basic operation
	...
	next
		...
		do
			...
			if index <= count and index <= chunk_source_upper then
				i := 1 + index - chunk_source_lower - 1 -- -1 as SPECIAL starts at index 0!!!
				...
				if last_byte = 10 then -- '%N' = 10 = 0xOA
					...
					-- End of input ?
				check index > count or else current_chunk.count = 0 and end_of_input end
				c := 0 -- '%U'
				column := column + 1
			end
			last_byte := c
...
feature {NONE} -- Implementation
	...
	get_next_chunk
		...
		do
			if source.file_readable then
				...
				current_chunk := to_byte_array (source.last_string.area)
			else
				create current_chunk.make_empty (0)
			end
	...
	current_chunk: SPECIAL [NATURAL_8]
			-- Current chunk
...
feature {NONE} -- Implementation		
	...
	to_byte_array (sp: SPECIAL [CHARACTER_8]): SPECIAL [NATURAL_8]
		local
			mp: MANAGED_POINTER
		do
			create mp.make_from_pointer (sp.item_address (0), sp.count)
			Result := mp.read_special_natural_8 (0, sp.count - 1)
		end
