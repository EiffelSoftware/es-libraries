note
	description: "[
		Windows-specific tests for BCRYPT_RNG.
		
		Tests cover:
		- BCrypt RNG initialization and generation
...
feature -- BCRYPT_RNG Tests

	test_bcrypt_rng_init
			-- Verify BCRYPT_RNG initializes and generates without error.
		local
			rng: BCRYPT_RNG
			v1, v2: NATURAL_32
		do
	...
	test_bcrypt_rng_fill_bytes
			-- Verify BCRYPT_RNG fills buffer correctly.
		local
			rng: BCRYPT_RNG
	...
	test_bcrypt_is_secure
			-- Verify BCRYPT_RNG reports as cryptographically secure.
		local
			rng: BCRYPT_RNG
		do
			create rng.make
	...
	test_bcrypt_error_reporting
			-- Verify BCRYPT_RNG provides error status and messages.
		local
			rng: BCRYPT_RNG
		do
			create rng.make
	...
	test_bcrypt_derived_primitives
			-- Verify BCRYPT_RNG derived primitives work correctly.
		local
			rng: BCRYPT_RNG
			u64: NATURAL_64
			byte: NATURAL_8
	...
	test_bcrypt_entropy_source
			-- Verify BCRYPT_RNG reports correct entropy source.
		local
			rng: BCRYPT_RNG
		do
			create rng.make
			assert ("Entropy source not empty", not rng.entropy_source.is_empty)
			assert ("Contains BCrypt", rng.entropy_source.has_substring ("BCrypt"))
		end

	test_bcrypt_multiple_generations
	...
	test_bcrypt_entropy_distribution
