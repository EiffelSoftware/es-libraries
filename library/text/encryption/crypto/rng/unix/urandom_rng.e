note
    description: "[
        Cryptographically secure RNG using /dev/urandom (Unix/Linux/macOS).
        ...
        Not available on Windows - use BCRYPT_RNG instead.
        
        On OpenBSD, this internally uses arc4random_buf which is the
        recommended method per OpenBSD documentation.
        
        Error codes:
        -  0: Success
        - -1: Failed to open /dev/urandom
        - -2: Failed to read from device
        - -3: API call failed (unused on Unix)
        - -4: Unsupported platform
        
        Security: On any error, the output buffer is automatically zeroed
        to prevent information leakage.
        
        Thread-safe. Both getrandom() and /dev/urandom reads are thread-safe.
		
		Implements CSPRNG recommendation from Practical Cryptography for Developers:
        'In Linux and macOS, it is considered that both /dev/random and /dev/urandom
        sources of randomness are secure enough for most cryptographic purposes.'
    ...
    error_message: STRING_8
        ...
        do
            ...
            when Error_open then
                Result := "Failed to open /dev/urandom"
            when Error_read then
                Result := "Failed to read random data from /dev/urandom"
            when Error_api then
                Result := "API call failed"
            when Error_unsupported then
                Result := "Platform not supported (not Unix/Linux/macOS/OpenBSD)"
            else
...
feature -- Core primitive
    ...
    next_u32: NATURAL_32
        ...
        do
            ...
            last_error_code := c_fill_bytes_urandom (l_buffer.item, Bytes_per_u32)
    ...
    fill_bytes (a_buffer: SPECIAL [NATURAL_8])
        ...
        do
            if a_buffer.count > 0 then
                ...
                last_error_code := c_fill_bytes_urandom (l_ptr.item, a_buffer.count)
                ...
                -- On error, buffer retains previous content (caller checks has_error)
            end
        end
...
feature -- Information
    ...
    entropy_source: STRING_8
        ...
        do
            Result := "Unix getrandom()/dev/urandom"
        end
...
feature {NONE} -- External C
    ...
    c_fill_bytes_urandom (ptr: POINTER; size: INTEGER): INTEGER
        ...
        alias
            "[
                return csprng_fill_bytes_urandom(
                    (unsigned char *)$ptr,
                    (size_t)$size
                );
            ]"
