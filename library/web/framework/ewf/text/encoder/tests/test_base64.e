...
feature -- Tests
	...
	test_valid_64_encoding
		do
			assert ("Expected encoded True:", is_valid_base64_encoding ((create {BASE64_ENCODER}).encoded_string ("content")))
		end
	...
	is_valid_base64_encoding (a_string: STRING): BOOLEAN
		...
		do
			if not l_retry then
				...
				Result := not l_encoder.has_error and not l_encoder.has_incorrect_padding
			end
		rescue
			l_retry := True
			retry
