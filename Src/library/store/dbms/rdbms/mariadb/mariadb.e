note
	description: "MariaDB specification"
...
feature -- Constants
	...
	database_handle_name: STRING = "MARIADB"
	...
	connect (user_name, user_passwd, data_source, application, hostname,
		...
		do
			...
			if l_colon_position > 0 then
				create l_host.make (hostname.substring (1, l_colon_position - 1))
				l_port := hostname.substring (l_colon_position + 1, hostname.count).to_integer
			else
...
feature {NONE} -- Implementation
	...
	fetch_result_columns (no_descriptor: INTEGER; a_results: DB_PARA_MARIADB)
...
feature {NONE} -- Attributes
	...
	arguments: ARRAY [detachable DB_PARA_MARIADB]
			-- Array of parameters to be bound for dynamic SQL.

	results: ARRAY [detachable DB_PARA_MARIADB]
