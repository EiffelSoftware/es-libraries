		...
		do
			...
			if applicable_attributes.font_family then
				...
				if a_text_tag = default_pointer then
					...
					{GOBJECT}.g_object_set_string (a_text_tag, {GTK_PROPERTIES}.family, a_text_tag_name.item)
			...
			if applicable_attributes.font_shape then
				...
				if a_text_tag = default_pointer then
					a_text_tag := {GTK2}.gtk_text_tag_new (a_text_tag_name.item)
					{GOBJECT}.g_object_set_integer (a_text_tag, {GTK_PROPERTIES}.style, prop_value_int)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
			...
			if applicable_attributes.font_weight then
				...
				if a_text_tag = default_pointer then
					a_text_tag := {GTK2}.gtk_text_tag_new (a_text_tag_name.item)
					{GOBJECT}.g_object_set_integer (a_text_tag, {GTK_PROPERTIES}.weight, prop_value_int)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
			...
			if applicable_attributes.color then
				...
				if a_text_tag = default_pointer then
					...
					{GOBJECT}.g_object_set_pointer (a_text_tag, {GTK_PROPERTIES}.foreground_rgba, App_implementation.reusable_rgba_struct)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
			...
			if applicable_attributes.background_color then
				...
				if a_text_tag = default_pointer then
					...
					{GOBJECT}.g_object_set_pointer (a_text_tag, {GTK_PROPERTIES}.background_rgba, App_implementation.reusable_rgba_struct)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
			...
			if applicable_attributes.effects_striked_out then
				...
				if a_text_tag = default_pointer then
					a_text_tag := {GTK2}.gtk_text_tag_new (a_text_tag_name.item)
					{GOBJECT}.g_object_set_boolean (a_text_tag, {GTK_PROPERTIES}.strikethrough, is_striked_out)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
			...
			if applicable_attributes.effects_underlined then
				...
				if a_text_tag = default_pointer then
					a_text_tag := {GTK2}.gtk_text_tag_new (a_text_tag_name.item)
					{GOBJECT}.g_object_set_boolean (a_text_tag, {GTK_PROPERTIES}.underline, is_underlined)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
			...
			if applicable_attributes.effects_vertical_offset then
				...
				if a_text_tag = default_pointer then
					a_text_tag := {GTK2}.gtk_text_tag_new (a_text_tag_name.item)
					{GOBJECT}.g_object_set_integer (a_text_tag, {GTK_PROPERTIES}.rise, vertical_offset)
					{GTK2}.gtk_text_tag_table_add (a_tag_table, a_text_tag)
				end
				{GTK2}.gtk_text_buffer_apply_tag (a_text_buffer, a_text_tag, a_start_iter, a_end_iter)
			end
	...
	new_text_tag_from_applicable_attributes (applicable_attributes: EV_CHARACTER_FORMAT_RANGE_INFORMATION): POINTER
		...
		do
			...
			if (applicable_attributes.font_family or else applicable_attributes.font_height or else applicable_attributes.font_shape or else applicable_attributes.font_weight) then
				...
				if applicable_attributes.font_shape then
					if shape = {EV_FONT_CONSTANTS}.shape_italic then
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.style, 2)
					else
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.style, 0)
					end
				end
				if applicable_attributes.font_weight then
					inspect
					...
					when
						{EV_FONT_CONSTANTS}.weight_bold
					then
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.weight, {EV_FONT_IMP}.pango_weight_bold)
					when
						{EV_FONT_CONSTANTS}.weight_regular
					then
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.weight, {EV_FONT_IMP}.pango_weight_normal)
					when
						{EV_FONT_CONSTANTS}.weight_thin
					then
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.weight, {EV_FONT_IMP}.pango_weight_ultra_light)
					when
						{EV_FONT_CONSTANTS}.weight_black
					then
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.weight, {EV_FONT_IMP}.pango_weight_heavy)
					end
				end
			end
			...
			if applicable_attributes.color then
				...
				{GOBJECT}.g_object_set_string (Result, {GTK_PROPERTIES}.foreground, propvalue.item)
			end
			...
			if applicable_attributes.background_color then
				...
				temp_string.prepend_character ('#')
				propvalue := temp_string
				{GOBJECT}.g_object_set_string (Result, {GTK_PROPERTIES}.background, propvalue.item)
			end
			...
			if (applicable_attributes.effects_striked_out or else applicable_attributes.effects_underlined or else applicable_attributes.effects_vertical_offset) then
				if applicable_attributes.effects_striked_out then
					{GOBJECT}.g_object_set_boolean (Result, {GTK_PROPERTIES}.strikethrough, is_striked_out)
				end
				if applicable_attributes.effects_underlined then
					if is_underlined then
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.underline, {PANGO}.underline_single_enum)
					else
						{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.underline, {PANGO}.underline_none_enum)
					end
				end
				if applicable_attributes.effects_vertical_offset then
					{GOBJECT}.g_object_set_integer (Result, {GTK_PROPERTIES}.rise, vertical_offset)
				end
			end
		end
